...
async def reprocess_scenes_workflow(
    ...
    try:
        ...
        # Update video statistics in Firestore
        try:
            video_repo = VideoRepository(user_id)
            clips_repo = ClipRepository(user_id, run_id)
            
            # Update clip statistics (recalculates from all clips in Firestore)
            ...
            logger.info(f"Updated Firestore video {run_id} statistics after reprocessing")
        except Exception as e:
            # Log error but don't fail the reprocessing
            logger.error(
                f"Failed to update Firestore video statistics for {run_id}: {e}",
                exc_info=True,
            )
