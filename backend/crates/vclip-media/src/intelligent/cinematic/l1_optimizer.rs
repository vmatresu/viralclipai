...
impl L1TrajectoryOptimizer {
    ...
    pub fn optimize(&self, keyframes: &[CameraKeyframe]) -> Result<Vec<CameraKeyframe>, L1Error> {
        if keyframes.is_empty() { return Ok(Vec::new()); }
        if keyframes.len() < 3 { return Ok(self.linear_interpolate(keyframes)); }
        ...
        tracing::info!("L1 Optimization for {} frames took {:?}", n, start_time_total.elapsed());
    ...
    fn optimize_1d(&self, signal: &[f64]) -> Result<Vec<f64>, L1Error> {
        ...
        let mut z0 = vec![0.0; n]; let mut u0 = vec![0.0; n];
        let mut z1 = vec![0.0; n - 1]; let mut u1 = vec![0.0; n - 1];
        let mut z2 = vec![0.0; n - 2]; let mut u2 = vec![0.0; n - 2];
        let mut z3 = vec![0.0; n - 3]; let mut u3 = vec![0.0; n - 3];
        ...
        for _iter in 0..cfg.max_iterations {
            ...
            for i in 0..n { rhs[i] += signal[i] + z0[i] - u0[i]; }
            ...
            for i in 0..n {
                z0[i] = soft_threshold(p[i] - signal[i] + u0[i], cfg.lambda_position / cfg.admm_rho);
            }
            for i in 0..n - 1 {
                let dp = p[i+1] - p[i];
            ...
            for i in 0..n { u0[i] += p[i] - signal[i] - z0[i]; }
            for i in 0..n - 1 { u1[i] += p[i+1] - p[i] - z1[i]; }
            for i in 0..n - 2 { u2[i] += p[i+2] - 2.0*p[i+1] + p[i] - z2[i]; }
            for i in 0..n - 3 { u3[i] += p[i+3] - 3.0*p[i+2] + 3.0*p[i+1] - p[i] - z3[i]; }

            // Convergence Check
            let residual = p.iter().zip(p_old.iter())
...
impl SeptadiagonalSolver {
    fn new(n: usize) -> Self {
        if n == 0 { return Self { d: vec![], l1: vec![], l2: vec![], l3: vec![], n: 0 }; }
        ...
        // Term 0: Identity
        for i in 0..n { m0[i] += 1.0; }

        let mut add = |r: usize, c: usize, val: f64| {
            match c.checked_sub(r) {
                Some(0) => m0[r] += val,
                Some(1) => m1[r] += val,
                Some(2) => m2[r] += val,
                Some(3) => m3[r] += val,
        ...
        for k in 0..n.saturating_sub(1) {
            add(k, k, 1.0); add(k, k+1, -1.0); add(k+1, k+1, 1.0);
        }
        // Term 2: D2 [1, -2, 1]
        for k in 0..n.saturating_sub(2) {
            let s = [1.0, -2.0, 1.0];
            for i in 0..3 { for j in 0..3 { add(k+i, k+j, s[i]*s[j]); } }
        }
        // Term 3: D3 [-1, 3, -3, 1]
        for k in 0..n.saturating_sub(3) {
            let s = [-1.0, 3.0, -3.0, 1.0];
            for i in 0..4 { for j in 0..4 { add(k+i, k+j, s[i]*s[j]); } }
        }
        ...
        for i in 0..n {
            // D[i]
            let mut val = m0[i];
            if i > 0 { val -= d[i-1] * (l1[i-1] * l1[i-1]); }
            if i > 1 { val -= d[i-2] * (l2[i-2] * l2[i-2]); }
            if i > 2 { val -= d[i-3] * (l3[i-3] * l3[i-3]); }
            d[i] = val;
            let inv_d = 1.0 / val;
            ...
            if i + 1 < n {
                let mut v = m1[i];
                if i > 0 { v -= d[i-1] * l1[i-1] * l2[i-1]; }
                if i > 1 { v -= d[i-2] * l2[i-2] * l3[i-2]; }
                l1[i] = v * inv_d;
            }
            // L2[i] = L_{i+2, i}
            if i + 2 < n {
                let mut v = m2[i];
                if i > 0 { v -= d[i-1] * l1[i-1] * l3[i-1]; }
                l2[i] = v * inv_d;
            }
            // L3[i] = L_{i+3, i}
            if i + 3 < n {
    ...
    fn solve(&self, b: &[f64]) -> Vec<f64> {
        ...
        // Forward: L y = b
        for i in 0..n {
            if i >= 1 { x[i] -= self.l1[i-1] * x[i-1]; }
            if i >= 2 { x[i] -= self.l2[i-2] * x[i-2]; }
            if i >= 3 { x[i] -= self.l3[i-3] * x[i-3]; }
        }
        // Diagonal: D z = y
        for i in 0..n { x[i] /= self.d[i]; }
        // Backward: L^T x = z
        for i in (0..n).rev() {
            if i + 1 < n { x[i] -= self.l1[i] * x[i+1]; }
            if i + 2 < n { x[i] -= self.l2[i] * x[i+2]; }
            if i + 3 < n { x[i] -= self.l3[i] * x[i+3]; }
        }
        x
    }
}
...
mod tests {
    ...
    fn test_large_dataset_performance() {
        ...
        assert!(duration.as_millis() < 2000, "Optimization too slow! Took {:?}", duration);
    ...
    fn test_convergence_stability() {
        ...
        for i in 50..100 { signal[i] = 100.0; }
