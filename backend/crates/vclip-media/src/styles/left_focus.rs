...
impl LeftFocusProcessor {
    ...
    /// Get the FFmpeg filter for left focus processing.
    fn get_filter(&self) -> &'static str {
        filters::FILTER_LEFT_FOCUS
    }
...
impl StyleProcessor for LeftFocusProcessor {
    ...
    async fn process(&self, request: ProcessingRequest, ctx: ProcessingContext) -> MediaResult<ProcessingResult> {
        let timer = ctx.metrics.start_timer("left_focus_processing");
        let logger = ProcessingLogger::new(
            ctx.request_id.clone(),
            ctx.user_id.clone(),
            "left_focus".to_string(),
        );
        ...
        match ffmpeg_result {
            ...
            Ok(output) => {
                ...
                ctx.metrics.increment_counter("ffmpeg_error", &[("style", "left_focus")]);
                return Err(crate::error::MediaError::ffmpeg_failed(
                    "Left focus processing failed",
        ...
        ctx.metrics.increment_counter("processing_completed", &[("style", "left_focus")]);
