...
step_ipv6() {
    if [[ -z "$DOCKER_IPV6_SUBNET" ]]; then
        ...
        log_warn "To enable IPv6 rotation, use: --ipv6-subnet <cidr> (e.g., 2001:db8::/64)"
        return
    fi
...
# Auto-generated by setup-server.sh
IPV6_SUBNET_PREFIX="${DOCKER_IPV6_SUBNET%/*}"
...
EOF
    ...
    # 6. Create systemd service for IP assignment after Docker starts
...
ExecStart=/usr/local/bin/assign-ipv6.sh
RemainAfterExit=yes
...
EOF
    ...
    echo "To manually assign IPs: sudo /usr/local/bin/assign-ipv6.sh"
    echo "------------------------------------------------------------------"
}
