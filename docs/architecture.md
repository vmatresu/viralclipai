...
- **Tools**: Node-based worker utilities under `backend/tools` (youtubei.js transcript extraction)
...
tools/
  transcript-extractor # youtubei.js-based transcript tool
```

## Tools (Worker Runtime)

The worker bundles small Node-based utilities under `backend/tools` for
capabilities that are better maintained in JS (for example, youtubei.js
transcript extraction).

- Shared deps live in `backend/tools/package.json`.
- Each tool lives under `backend/tools/<tool-name>/`.
...
- **Coordinator**: `processor.rs` (no legacy suffix). Responsibilities:
  ...
  - Write `highlights.json` to R2 (analysis source of truth)
...
- **R2 layout**: `{user_id}/{video_id}/highlights.json` + `clips/*.mp4/.jpg` + transcript cache `{user_id}/transcripts/{cache_id}.txt.gz`
...
- youtubei.js transcript tool is invoked via Node and falls back to yt-dlp if unavailable.
